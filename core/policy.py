...
RUBRIC_VERSION = "v1"  # bump when scoring rubric / dimensions change
...
def bands_hash() -> str:
    """Short stable hash of BANDS, so an eval snapshot pins the exact
    thresholds it was run against, even if RUBRIC_VERSION wasn't bumped."""
    return hashlib.sha256(json.dumps(BANDS, sort_keys=True).encode()).hexdigest()[:12]
